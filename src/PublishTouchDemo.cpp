...
bool getParamsOfYaml(){
	...
	if(!ros::param::get("/publish_touch_demo/input_finger_id", input_finger_id)){
		ROS_WARN("input_finger_id param not found in param server! Using default.");
		input_finger_id = "1";
		success = false;
	
	if(!ros::param::get("/adaptive_grasp_controller/USE_SIGNATURE", input_synergy_threshold)){
		ROS_WARN("input_synergy_threshold param not found in param server! Using default.");
